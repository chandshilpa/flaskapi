...
def check_string(context, obj, stacklevel=3):
    if type(obj) is not str:
        warn(
            WSGIWarning(
                "%s requires bytestrings, got %s" % (context, obj.__class__.__name__)
            )
        )
...
class InputStream(object):
    ...
    def read(self, *args):
        if len(args) == 0:
            warn(
                WSGIWarning(
                    "wsgi does not guarantee an EOF marker on the "
    ...
    def readline(self, *args):
        if len(args) == 0:
            warn(
                WSGIWarning(
                    "Calls to wsgi.input.readline() without arguments"
            ...
            warn(
                WSGIWarning(
                    "wsgi.input.readline() was called with a size hint. "
...
class GuardedIterator(object):
    ...
    def next(self):
        ...
        if not self.headers_set:
            warn(
                WSGIWarning("Application returned before it " "started the response"),
                stacklevel=2,
            )
            ...
            if status_code == 304:
                for key, value in headers:
                    ...
                    if key not in ("expires", "content-location") and is_entity_header(
                        ...
                        warn(
                            HTTPWarning(
                                "entity header %r found in 304 " "response" % key
                            )
                        )
                if bytes_sent:
                ...
                if content_length != 0:
                    warn(
                        HTTPWarning(
                            "%r responses must have an empty "
                ...
                warn(
                    WSGIWarning(
                        "Content-Length and the number of bytes "
    ...
    def check_environ(self, environ):
        ...
        for key in (
            ...
            if key not in environ:
                warn(
                    WSGIWarning("required environment key %r not found" % key),
                    stacklevel=3,
                )
        ...
        if path_info[:1] != "/":
            warn(
                WSGIWarning("PATH_INFO does not start with a slash: %r" % path_info),
                stacklevel=3,
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        def checking_start_response(*args, **kwargs):
            if len(args) not in (2, 3):
                warn(
                    WSGIWarning(
                        "Invalid number of arguments: %s, expected "
                        "2 or 3" % len(args),
                        stacklevel=2,
                    )
                )
