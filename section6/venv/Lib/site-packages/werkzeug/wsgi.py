...
from werkzeug.http import http_date, is_resource_modified, is_hop_by_hop_header
...
class ProxyMiddleware(object):
    ...
    def __init__(self, app, targets, chunk_size=2 << 13, timeout=10):
        ...
        self.app = app
        self.targets = dict(
            ("/%s/" % k.strip("/"), _set_defaults(v)) for k, v in iteritems(targets)
        )
    ...
    def proxy_to(self, opts, path, prefix):
        ...
        def application(environ, start_response):
            ...
            try:
                ...
                con.connect()
                con.putrequest(
                    environ["REQUEST_METHOD"], url_quote(remote_path), skip_host=True
                )
