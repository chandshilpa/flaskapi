...
def File(fd, filename=None, mimetype=None):
    ...
    warn(
        DeprecationWarning(
            "werkzeug.test.File is deprecated, use the "
...
class EnvironBuilder(object):
    ...
    def _add_file_from_data(self, key, value):
        ...
        if isinstance(value, tuple):
            ...
            warn(
                DeprecationWarning(
                    "it's no longer possible to pass dicts "
    ...
    def _get_base_url(self):
        return (
            url_unparse((self.url_scheme, self.host, self.script_root, "", "")).rstrip(
                "/"
            )
            + "/"
        )
...
class Client(object):
    ...
    def resolve_redirect(self, response, new_location, environ, buffered=False):
        ...
        if not allowed:
            raise RuntimeError(
                "%r does not support redirect to " "external targets" % self.__class__
            )
