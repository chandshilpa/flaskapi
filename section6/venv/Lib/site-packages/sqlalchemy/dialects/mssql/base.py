...
class MSIdentifierPreparer(compiler.IdentifierPreparer):
    ...
    def quote_schema(self, schema, force=None):
        ...
        if dbname:
            result = "%s.%s" % (self.quote(dbname, force), self.quote(owner, force))
        elif owner:
...
class MSDialect(default.DefaultDialect):
    ...
    def has_table(self, connection, tablename, dbname, owner, schema):
        ...
        if owner:
            whereclause = sql.and_(whereclause, columns.c.table_schema == owner)
    ...
    def get_schema_names(self, connection, **kw):
        s = sql.select(
            [ischema.schemata.c.schema_name], order_by=[ischema.schemata.c.schema_name]
        )
    ...
    def get_columns(self, connection, tablename, dbname, owner, schema, **kw):
        ...
        cursor = connection.execute(
            "sp_columns @table_name = '%s', " "@table_owner = '%s'" % (tablename, owner)
        )
        ic = None
        while True:
            row = cursor.fetchone()
            ...
            if type_name.endswith("identity") and col_name in colmap:
                ...
                colmap[col_name]["sequence"] = dict(name="%s_identity" % col_name)
                break
        cursor.close()
        ...
        if ic is not None and self.server_version_info >= MS_2005_VERSION:
            ...
            if row is not None and row[0] is not None:
                colmap[ic]["sequence"].update(
                    {"start": int(row[0]), "increment": int(row[1])}
                )
        return cols
