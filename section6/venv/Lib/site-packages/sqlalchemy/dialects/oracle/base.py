    ...
    def visit_join(self, join, **kwargs):
        if self.dialect.use_ansi:
            ...
            if isinstance(join.right, expression.FromGrouping):
                ...
                right = join.right
            return (
                self.process(join.left, **kwargs) + ", " + self.process(right, **kwargs)
            )
        ...
        if not getattr(select, "_oracle_visit", None):
            ...
            if limit_clause is not None or offset_clause is not None:
                ...
                if offset_clause is None:
                    ...
                    if not self.dialect.use_binds_for_limits:
                        offset_clause = sql.literal_column("%d" % select._offset)
...
class OracleDialect(default.DefaultDialect):
    ...
    def _resolve_synonym(
        self, connection, desired_owner=None, desired_synonym=None, desired_table=None
    ):
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        constraint_data = self._get_constraint_data(
            connection, table_name, schema, dblink, info_cache=kw.get("info_cache")
        )
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        return [
            ...
            for name, cols in [
                [self.normalize_name(i[0]), [self.normalize_name(x[2]) for x in i[1]]]
