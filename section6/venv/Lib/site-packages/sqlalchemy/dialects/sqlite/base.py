...
colspecs = {sqltypes.Date: DATE, sqltypes.DateTime: DATETIME, sqltypes.Time: TIME}
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def __init__(self, isolation_level=None, native_datetime=False, **kwargs):
        ...
        if self.dbapi is not None:
            self.supports_right_nested_joins = self.dbapi.sqlite_version_info >= (
                3,
                7,
                16,
            )
    ...
    def get_view_definition(self, connection, view_name, schema=None, **kw):
        if schema is not None:
            ...
            try:
                ...
                s = (
                    "SELECT sql FROM sqlite_master WHERE name = '%s' " "AND type='view'"
                ) % view_name
                rs = connection.execute(s)
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        for row in info:
            ...
            columns.append(
                self._get_column_info(name, type_, nullable, default, primary_key)
            )
        return columns
    ...
    def _get_table_sql(self, connection, table_name, schema=None, **kw):
        ...
        try:
            s = (
                ...
                "WHERE name = '%(table)s' "
                "AND type = 'table'" % {"schema": schema_expr, "table": table_name}
            )
            rs = connection.execute(s)
        except exc.DBAPIError:
            s = (
