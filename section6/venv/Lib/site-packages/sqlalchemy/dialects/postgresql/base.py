    ...
    def get_select_precolumns(self, select, **kw):
        if select._distinct is not False:
            if select._distinct is True:
                ...
                return (
                    "DISTINCT ON ("
                    + ", ".join([self.process(col, **kw) for col in select._distinct])
                    + ") "
                )
            else:
                return "DISTINCT ON (" + self.process(select._distinct, **kw) + ") "
        else:
            return ""
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_exclude_constraint(self, constraint, **kw):
        ...
        for expr, name, op in constraint._render_exprs:
            ...
            elements.append("%s WITH %s" % (self.sql_compiler.process(expr, **kw), op))
...
class PGDialect(default.DefaultDialect):
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        # format columns
        columns = []
        for name, format_type, default, notnull, attnum, table_oid, comment in rows:
            column_info = self._get_column_info(
                name, format_type, default, notnull, domains, enums, schema, comment
            )
            columns.append(column_info)
        return columns

    def _get_column_info(
        self, name, format_type, default, notnull, domains, enums, schema, comment
    ):
    ...
    def get_foreign_keys(
        ...
        **kw
        ...
        t = sql.text(
            FK_SQL, typemap={"conname": sqltypes.Unicode, "condef": sqltypes.Unicode}
        )
    ...
    def get_indexes(self, connection, table_name, schema, **kw):
        ...
        t = sql.text(
            IDX_SQL, typemap={"relname": sqltypes.Unicode, "attname": sqltypes.Unicode}
        )
        c = connection.execute(t, table_oid=table_oid)
        ...
        for row in c.fetchall():
            ...
            if prd and not idx_name == sv_idx_name:
                util.warn(
                    "Predicate of partial index %s ignored during reflection" % idx_name
                )
                sv_idx_name = idx_name
    ...
    def get_check_constraints(self, connection, table_name, schema=None, **kw):
        ...
        return [{"name": name, "sqltext": src[1:-1]} for name, src in c.fetchall()]
    ...
    def _load_enums(self, connection, schema=None):
        ...
        s = sql.text(
            SQL_ENUMS, typemap={"attname": sqltypes.Unicode, "label": sqltypes.Unicode}
        )
