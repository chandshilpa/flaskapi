...
class Numeric(_LookupExpressionAdapter, TypeEngine):
    ...
    def __init__(
        self, precision=None, scale=None, decimal_return_scale=None, asdecimal=True
    ):
        """
        Construct a Numeric.
...
class Float(Numeric):
    ...
    def __init__(
        self, precision=None, asdecimal=False, decimal_return_scale=None, **kwargs
    ):
        r"""
        Construct a Float.
...
class SchemaType(SchemaEventTarget):
    ...
    def _is_impl_for_variant(self, dialect, kw):
        ...
        if dialect.name in variant_mapping and variant_mapping[dialect.name] is self:
            return True
...
class Enum(Emulated, String, SchemaType):
    ...
    def _enum_init(self, enums, kw):
        ...
        super(Enum, self).__init__(length=length, convert_unicode=convert_unicode)
    ...
    def _parse_into_values(self, enums, kw):
        ...
        if len(enums) == 1 and hasattr(enums[0], "__members__"):
            ...
            objects = [
                self.enum_class.__members__[k] for k in self.enum_class.__members__
            ]
    ...
    def __repr__(self):
        return util.generic_repr(
            self, additional_kw=[("native_enum", True)], to_inspect=[Enum, SchemaType]
        )
...
class PickleType(TypeDecorator):
    ...
    def __init__(self, protocol=pickle.HIGHEST_PROTOCOL, pickler=None, comparator=None):
        """
    ...
    class Comparator(Indexable.Comparator, Concatenable.Comparator):
        ...
        def _setup_getitem(self, index):
            if isinstance(index, slice):
                ...
                index = Slice(
                    _literal_as_binds(
                        index.start, name=self.expr.key, type_=type_api.INTEGERTYPE
                    ),
                    _literal_as_binds(
                        index.stop, name=self.expr.key, type_=type_api.INTEGERTYPE
                    ),
                    _literal_as_binds(
                        index.step, name=self.expr.key, type_=type_api.INTEGERTYPE
                    ),
                )
            else:
                if self.type.zero_indexes:
