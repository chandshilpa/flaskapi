...
def _append_param_insert_pk_returning(compiler, stmt, c, values, kw):
    ...
    if c.default is not None:
        if c.default.is_sequence:
            ...
            values.append((c, compiler.process(c.default.arg.self_group(), **kw)))
            compiler.returning.append(c)
        else:
...
def _append_param_update(compiler, stmt, c, implicit_return_defaults, values, kw):
    ...
    if c.onupdate is not None and not c.onupdate.is_sequence:
        if c.onupdate.is_clause_element:
            values.append((c, compiler.process(c.onupdate.arg.self_group(), **kw)))
            if implicit_return_defaults and c in implicit_return_defaults:
                compiler.returning.append(c)
            else:
                compiler.postfetch.append(c)
...
def _get_returning_modifiers(compiler, stmt):
    ...
    return need_pks, implicit_returning, implicit_return_defaults, postfetch_lastrowid
