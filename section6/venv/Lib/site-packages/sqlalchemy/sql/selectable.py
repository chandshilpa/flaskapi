...
class FromClause(Selectable):
    ...
    * Methods to generate various derivations of a "from" clause, including
    ...
    c = property(
        attrgetter("columns"), doc="An alias for the :attr:`.columns` attribute."
    )
...
class ForUpdateArg(ClauseElement):
    ...
    def __init__(
        self, nowait=False, read=False, of=None, skip_locked=False, key_share=False
    ):
...
class GenerativeSelect(SelectBase):
    ...
    def __init__(
        ...
        if group_by is not None:
            self._group_by_clause = ClauseList(
                *util.to_list(group_by), _literal_as_text=_literal_as_label_reference
            )
    ...
    def append_order_by(self, *clauses):
        ...
        if len(clauses) == 1 and clauses[0] is None:
            ...
            self._order_by_clause = ClauseList(
                *clauses, _literal_as_text=_literal_and_labels_as_label_reference
            )
...
class Select(HasPrefixes, HasSuffixes, GenerativeSelect):
    ...
    def _froms(self):
        ...
        for item in itertools.chain(
            ...
            _from_objects(self._whereclause) if self._whereclause is not None else (),
            self._from_obj,
        ):
            if item is self:
    ...
    def _get_display_froms(
        ...
        if self._auto_correlate and implicit_correlate_froms and len(froms) > 1:
    ...
    def _copy_internals(self, clone=_clone, **kw):
        ...
        self._from_obj = util.OrderedSet(from_cloned[f] for f in self._from_obj)
        ...
        self._correlate = set(clone(f) for f in self._correlate).union(self._correlate)
