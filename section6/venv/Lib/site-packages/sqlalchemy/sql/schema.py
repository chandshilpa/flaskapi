...
class Table(DialectKWArgs, SchemaItem, TableClause):
    ...
    def get_children(self, column_collections=True, schema_visitor=False, **kw):
    ...
    def tometadata(
        self, metadata, schema=RETAIN_SCHEMA, referred_schema_fn=None, name=None
    ):
...
class Column(SchemaItem, ColumnClause):
    ...
    def copy(self, **kw):
        ...
        args = [c.copy(**kw) for c in self.constraints if not c._type_bound] + [
...
class PassiveDefault(DefaultClause):
    ...
    @util.deprecated(
        "0.6",
        ":class:`.PassiveDefault` is deprecated.  " "Use :class:`.DefaultClause`.",
        False,
    )
        ...
        if len(tables) == 1:
            ...
            if others:
                raise exc.ArgumentError(
                    ...
                    % (", ".join("'%s'" % c for c in others), table.description)
                )
    ...
    def __init__(
        ...
        if reflect:
            util.warn_deprecated(
                "reflect=True is deprecate; please " "use the reflect() method."
            )
            if not bind:
                raise exc.ArgumentError(
    ...
    def __init__(self, map_):
        ...
        if map_ is not None:
            ...
            def schema_for_object(obj):
                ...
                effective_schema = obj._translate_schema(effective_schema, map_)
