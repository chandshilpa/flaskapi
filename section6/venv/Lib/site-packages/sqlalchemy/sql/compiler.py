...
class Compiled(object):
    ...
    @util.deprecated(
        "0.7", ":class:`.Compiled` objects now compile " "within the constructor."
    )
    def compile(self):
...
class SQLCompiler(Compiled):
    ...
    def __init__(self, dialect, statement, column_keys=None, inline=False, **kwargs):
        ...
        if (self.isinsert or self.isupdate or self.isdelete) and statement._returning:
    ...
    def visit_bindparam(
        ...
        **kwargs
        ...
        return self.bindparam_string(name, expanding=bindparam.expanding, **kwargs)
        ...
        if is_new_cte:
            ...
            if not cte_pre_alias_name and cte not in self.ctes:
                ...
                if cte.recursive:
                    ...
                    text += "(%s)" % (
                        ", ".join(
                            self.preparer.format_column(ident) for ident in recur_cols
                        )
                    )
    ...
    def _label_select_column(
        ...
        if not within_columns_clause:
            ...
            result_expr = _CompileLabel(col_expr, name, alt_names=(column._key_label,))
    ...
    def _transform_result_map_for_nested_joins(self, select, transformed_select):
        inner_col = dict((c._key_label, c) for c in transformed_select.inner_columns)
    ...
    def visit_update(self, update_stmt, asfrom=False, **kw):
        ...
        text += " SET "
        include_table = is_multitable and self.render_table_with_column_in_update_from
        text += ", ".join(
            c[0]._compiler_dispatch(self, include_table=include_table) + "=" + c[1]
...
class DDLCompiler(Compiled):
    ...
    def visit_drop_index(self, drop):
        index = drop.element
        return "\nDROP INDEX " + self._prepared_index_name(index, include_schema=True)
    ...
    def _prepared_index_name(self, index, include_schema=False):
        ...
        if isinstance(ident, elements._truncated_label):
            max_ = (
                self.dialect.max_index_name_length or self.dialect.max_identifier_length
            )
            if len(ident) > max_:
                ident = ident[0 : max_ - 8] + "_" + util.md5_hex(ident)[-4:]
        else:
...
class IdentifierPreparer(object):
    ...
    def format_column(
        self, column, use_table=False, name=None, table_name=None, use_schema=False
    ):
