...
class ClauseElement(Visitable):
    ...
    def _negate(self):
        return UnaryExpression(
            self.self_group(against=operators.inv), operator=operators.inv, negate=None
        )
...
class ColumnElement(operators.ColumnOperators, ClauseElement):
    ...
    def base_columns(self):
        return util.column_set(c for c in self.proxy_set if not hasattr(c, "_proxies"))
...
class BindParameter(ColumnElement):
    ...
    def _clone(self):
        ...
        if self.unique:
            c.key = _anonymous_label("%%(%d %s)s" % (id(c), c._orig_key or "param"))
        return c
    ...
    def __repr__(self):
        return "BindParameter(%r, %r, type_=%r)" % (self.key, self.value, self.type)
...
class ClauseList(ClauseElement):
    ...
    def __init__(self, *clauses, **kwargs):
        ...
        text_converter = kwargs.pop("_literal_as_text", _expression_literal_as_text)
...
class BooleanClauseList(ClauseList, ColumnElement):
    ...
    def _construct(cls, operator, continue_on, skip_on, *clauses, **kw):
        ...
        convert_clauses = [c.self_group(against=operator) for c in convert_clauses]
...
class Tuple(ClauseList, ColumnElement):
    ...
    def __init__(self, *clauses, **kw):
        ...
        self.type = kw.pop(
            "type_", self._type_tuple[0] if self._type_tuple else type_api.NULLTYPE
        )
...
class Grouping(ColumnElement):
    ...
    def compare(self, other, **kw):
        return isinstance(other, Grouping) and self.element.compare(other.element)
...
class Over(ColumnElement):
        ...
        * RANGE BETWEEN 1 FOLLOWING AND 3 FOLLOWING::
        ...
        if order_by is not None:
            self.order_by = ClauseList(
                *util.to_list(order_by), _literal_as_text=_literal_as_label_reference
            )
    ...
    def _interpret_range(self, range_):
        ...
        if range_[0] is None:
            ...
            try:
                ...
                raise exc.ArgumentError("Integer or None expected for range value")
            else:
                if lower == 0:
                    lower = RANGE_CURRENT
    ...
    def get_children(self, **kwargs):
        return [
            c for c in (self.element, self.partition_by, self.order_by) if c is not None
        ]
...
class Label(ColumnElement):
    ...
    def _copy_internals(self, clone=_clone, anonymize_labels=False, **kw):
        ...
        if anonymize_labels:
            self.name = self._resolve_label = _anonymous_label(
                "%%(%d %s)s" % (id(self), getattr(self.element, "name", "anon"))
            )
            self.key = self._label = self._key_label = self.name
    ...
    def _make_proxy(self, selectable, name=None, **kw):
        e = self.element._make_proxy(selectable, name=name if name else self.name)
    ...
    def _compare_name_for_result(self, other):
        if (
            ...
            or (
                ...
                and (other.is_literal or other.table is None or other.table._textual)
            )
        ):
...
class _anonymous_label(_truncated_label):
    ...
    def __add__(self, other):
        return _anonymous_label(
            quoted_name(util.text_type.__add__(self, util.text_type(other)), self.quote)
        )

    def __radd__(self, other):
        return _anonymous_label(
            quoted_name(util.text_type.__add__(util.text_type(other), self), self.quote)
        )
...
def _labeled(functions, element):
    if not hasattr(element, "name") or isinstance(element, functions.FunctionElement):
