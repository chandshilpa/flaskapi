        ...
        def __getitem__(self, key):
            try:
                ...
                if isinstance(key, slice):
                    l = []
                    for processor, value in zip(self._processors[key], self._row[key]):
                        if processor is None:
                            l.append(value)
                        else:
...
class ResultMetaData(object):
    ...
    def __init__(self, parent, cursor_description):
        ...
        if context.result_column_struct:
            ...
            result_columns = cols_are_ordered = num_ctx_cols = textual_ordered = False
        ...
        if not _baserowproxy_usecext:
            ...
            self._keymap.update(
                ...
                + [(elem[0] - len_raw, (elem[3], elem[4], elem[0])) for elem in raw]
            )
        ...
        if (
            ...
            return [
                (
                    ...
                    colname,
                    colname,
                    context.get_result_processor(mapped_type, colname, coltype),
                    obj,
                    untranslated,
                )
    ...
    def _key_fallback(self, key, raiseerr=True):
        ...
        # pickle/unpickle roundtrip
            ...
            if (
                key._label
                and (key._label if self.case_sensitive else key._label.lower()) in map
            ):
                result = map[key._label if self.case_sensitive else key._label.lower()]
            elif (
                hasattr(key, "name")
                and (key.name if self.case_sensitive else key.name.lower()) in map
            ):
                # match is only on name.
                result = map[key.name if self.case_sensitive else key.name.lower()]
    ...
    def _getter(self, key, raiseerr=True):
        ...
        if index is None:
            raise exc.InvalidRequestError(
                "Ambiguous column name '%s' in " "result set column descriptions" % obj
            )
...
class BufferedRowResultProxy(ResultProxy):
    ...
    # is reached
    size_growth = {1: 5, 5: 10, 10: 20, 20: 50, 50: 100, 100: 250, 250: 500, 500: 1000}
...
class BufferedColumnRow(RowProxy):
    def __init__(self, parent, row, processors, keymap):
        ...
        for processor in parent._orig_processors:
            ...
            index += 1
        row = tuple(row)
        super(BufferedColumnRow, self).__init__(parent, row, processors, keymap)
