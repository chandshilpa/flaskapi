...
class Connection(Connectable):
    ...
    def closed(self):
        ...
        return (
            "_Connection__connection" not in self.__dict__ and not self.__can_reconnect
        )
    ...
    def _execute_default(self, default, multiparams, params):
        ...
        if self._has_events or self.engine._has_events:
            self.dispatch.after_execute(self, default, multiparams, params, ret)
    ...
    def _execute_ddl(self, ddl, multiparams, params):
        ...
        ret = self._execute_context(
            dialect, dialect.execution_ctx_cls._init_ddl, compiled, None, compiled
        )
        if self._has_events or self.engine._has_events:
    ...
    def _execute_compiled(self, compiled, multiparams, params):
        ...
        if self._has_events or self.engine._has_events:
            self.dispatch.after_execute(self, compiled, multiparams, params, ret)
        return ret
    ...
    def _execute_text(self, statement, multiparams, params):
        ...
        if self._has_events or self.engine._has_events:
            self.dispatch.after_execute(self, statement, multiparams, params, ret)
        return ret
        ...
        if self._has_events or self.engine._has_events:
            for fn in self.dispatch.before_cursor_execute:
                statement, parameters = fn(
                    self, cursor, statement, parameters, context, context.executemany
                )
        ...
        if not self._is_disconnect:
            self._is_disconnect = (
                ...
                and self.dialect.is_disconnect(
                    e, self.__connection if not self.invalidated else None, cursor
                )
    ...
    def _handle_dbapi_exception_noconnection(cls, e, dialect, engine):
        ...
        if should_wrap:
            sqlalchemy_exception = exc.DBAPIError.instance(
                None, None, e, dialect.dbapi.Error, connection_invalidated=is_disconnect
            )
        else:
            sqlalchemy_exception = None
...
class Engine(Connectable, log.Identified):
    ...
    def raw_connection(self, _connection=None):
        ...
        """
        return self._wrap_pool_connect(self.pool.unique_connection, _connection)
