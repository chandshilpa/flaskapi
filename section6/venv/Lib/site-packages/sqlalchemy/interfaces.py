...
class PoolListener(object):
    ...
    def _adapt_listener(cls, self, listener):
        ...
        listener = util.as_interface(
            listener, methods=("connect", "first_connect", "checkout", "checkin")
        )
    ...
    def _adapt_listener(cls, self, listener):
        ...
        def adapt_cursor_execute(
            ...
            return listener.cursor_execute(
                execute_wrapper, cursor, statement, parameters, context, executemany
            )
        ...
        event.listen(
            self, "rollback_savepoint", adapt_listener(listener.rollback_savepoint)
        )
        event.listen(
            self, "release_savepoint", adapt_listener(listener.release_savepoint)
        )
        event.listen(self, "begin_twophase", adapt_listener(listener.begin_twophase))
        event.listen(
            self, "prepare_twophase", adapt_listener(listener.prepare_twophase)
        )
        event.listen(
            self, "rollback_twophase", adapt_listener(listener.rollback_twophase)
        )
        event.listen(self, "commit_twophase", adapt_listener(listener.commit_twophase))
