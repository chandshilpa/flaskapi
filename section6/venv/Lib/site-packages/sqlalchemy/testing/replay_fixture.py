...
class ReplayFixtureTest(fixtures.TestBase):
    ...
    def test_invocation(self):
        ...
        engine = create_engine(config.db.url, creator=recorder, use_native_hstore=False)
        ...
        engine = create_engine(config.db.url, creator=player, use_native_hstore=False)
...
class ReplayableSession(object):
    ...
    if util.py2k:
        ...
        Natives = (
            set([getattr(types, t) for t in dir(types) if not t.startswith("_")])
            .union(
                [
    ...
    class Recorder(object):
        ...
        def __call__(self, *args, **kw):
            subject, buffer = [
                object.__getattribute__(self, x) for x in ("_subject", "_buffer")
            ]
