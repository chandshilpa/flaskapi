...
class LimitOffsetTest(fixtures.TablesTest):
    ...
    def test_simple_limit(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id).limit(2), [(1, 1, 2), (2, 2, 3)]
        )
    ...
    def test_simple_offset(self):
        table = self.tables.some_table
        self._assert_result(
            select([table]).order_by(table.c.id).offset(2), [(3, 3, 4), (4, 4, 5)]
        )
...
class ExpandingBoundInTest(fixtures.TablesTest):
    ...
    def test_bound_in_two_tuple(self):
        ...
        stmt = (
            select([table.c.id])
            .where(tuple_(table.c.x, table.c.y).in_(bindparam("q", expanding=True)))
            .order_by(table.c.id)
        )
