...
class BakedQuery(object):
    ...
    def _add_lazyload_options(self, options, effective_path, cache_path=None):
        ...
        self.add_criteria(
            lambda q: q._with_current_path(effective_path)._conditional_options(
                *options
            ),
            cache_path.path,
            key,
    ...
    def _unbake_subquery_loaders(self, session, context, params, post_criteria):
        ...
        for k, cache_key, query in context.attributes["baked_queries"]:
            bk = BakedQuery(self._bakery, lambda sess, q=query: q.with_session(sess))
...
@util.deprecated(
    "1.2", "Baked lazy loading is now the default " "implementation for lazy loading."
)
