        ...
        if typecolname is not None:
            result.append(
                sql.select(
                    ...
                    + [
                        sql.literal_column(sql_util._quote_ddl_expr(type)).label(
                            typecolname
                        )
                    ],
                    from_obj=[table],
            ...
            result.append(
                sql.select([col(name, table) for name in colnames], from_obj=[table])
            )
...
def aliased(element, alias=None, name=None, flat=False, adapt_on_names=False):
    ...
    if isinstance(element, expression.FromClause):
        ...
        return AliasedClass(
            element, alias=alias, flat=flat, name=name, adapt_on_names=adapt_on_names
        )
...
class _ORMJoin(expression.Join):
    ...
    def join(self, right, onclause=None, isouter=False, full=False, join_to_left=None):
