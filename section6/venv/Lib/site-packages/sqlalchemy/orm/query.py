...
class Query(object):
    ...
    def _set_select_from(self, obj, set_base_alias):
        ...
        for from_obj in obj:
            ...
            if hasattr(info, "mapper") and (info.is_mapper or info.is_aliased_class):
    ...
    def _adapt_col_list(self, cols):
        return [
            self._adapt_clause(expression._literal_as_label_reference(o), True, True)
            for o in cols
        ]
    ...
    def with_polymorphic(self, cls_or_mappers, selectable=None, polymorphic_on=None):
        ...
        entity.set_with_polymorphic(
            self, cls_or_mappers, selectable=selectable, polymorphic_on=polymorphic_on
        )
        ...
        if property is None:
            ...
            for prop in mapper.iterate_properties:
                ...
                raise sa_exc.InvalidRequestError(
                    ...
                    % (entity_zero.mapper.class_.__name__, instance.__class__.__name__)
                )
    ...
    def from_self(self, *entities):
        ...
        **Looking out for Inner / Outer Columns**
        ...
        """
        fromclause = (
            self.with_labels().enable_eagerloads(False).statement.correlate(None)
        )
    ...
    @_generative()
    def with_for_update(
        self, read=False, nowait=False, of=None, skip_locked=False, key_share=False
    ):
    ...
    def _join(self, keys, outerjoin, full, create_aliases, from_joinpoint):
        ...
        if (
            len(keys) == 2
            and isinstance(keys[0], (expression.FromClause, type, AliasedClass))
            and isinstance(
                keys[1], (str, expression.ClauseElement, interfaces.PropComparator)
            )
        ):
        ...
        for idx, arg1 in enumerate(keylist):
            ...
            if isinstance(onclause, interfaces.PropComparator):
                ...
                if isinstance(alias, ORMAdapter) and alias.mapper.isa(left_entity):
    ...
    def _join_left_to_right(
        self, left, right, onclause, outerjoin, full, create_aliases, prop
    ):
        ...
        right, onclause = self._prepare_right_side(
            r_info, right, onclause, create_aliases, prop, overlap
        )
    ...
    def _prepare_right_side(
        self, r_info, right, onclause, create_aliases, prop, overlap
    ):
        info = r_info
        ...
        if aliased_entity and not create_aliases:
            self._mapper_loads_polymorphically_with(
                right_mapper,
                ORMAdapter(right, equivalents=right_mapper._equivalent_columns),
            )
    ...
    def _join_to_left(self, l_info, left, right, onclause, outerjoin, full):
        ...
        if self._from_obj:
            replace_clause_index, clause = sql_util.find_join_source(
                self._from_obj, left_selectable
            )
        ...
        try:
            clause = orm_join(clause, right, onclause, isouter=outerjoin, full=full)
        except sa_exc.ArgumentError as ae:
            raise sa_exc.InvalidRequestError(
                "Could not find a FROM clause to join from.  "
                "Tried joining to %s, but got: %s" % (right, ae)
    ...
    def from_statement(self, statement):
        ...
        if not isinstance(statement, (expression.TextClause, expression.SelectBase)):
            raise sa_exc.ArgumentError(
                "from_statement accepts text(), select(), " "and union() objects only."
            )
    ...
    def _get_bind_args(self, querycontext, fn, **kw):
        return fn(mapper=self._bind_mapper(), clause=querycontext.statement, **kw)
...
class LockmodeArg(ForUpdateArg):
    ...
    def parse_legacy_query(self, mode):
        ...
        if mode == "read":
            ...
            raise sa_exc.ArgumentError("Unknown with_lockmode argument: %r" % mode)
...
class _QueryEntity(object):
    ...
    def __new__(cls, *args, **kwargs):
        if cls is _QueryEntity:
            entity = args[1]
            if not isinstance(entity, util.string_types) and _is_mapped_class(entity):
                cls = _MapperEntity
...
class _BundleEntity(_QueryEntity):
    ...
    def row_processor(self, query, context, result):
        procs, labels = zip(
            *[ent.row_processor(query, context, result) for ent in self._entities]
        )
