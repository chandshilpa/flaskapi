...
from .base import _is_aliased_class, _class_to_mapper, _is_mapped_class, InspectionAttr
from . import util as orm_util
from .path_registry import PathRegistry, TokenRegistry, _WILDCARD_TOKEN, _DEFAULT_TOKEN
...
class Load(Generative, MapperOption):
    ...
    def _set_path_strategy(self):
        ...
        self._set_for_path(
            self.context, effective_path, replace=True, merge_opts=self.is_opts_only
        )
        ...
        for token in path:
            if isinstance(token, QueryableAttribute):
                if (
                    ...
                    ret.append((token._parentmapper.class_, token.key, token._of_type))
    ...
    def _bind_loader(self, entities, current_path, context, raiseerr):
        ...
        if not loader.is_class_strategy:
            for token in start_path:
                if not loader._generate_path(loader.path, token, None, raiseerr):
                    return
...
def defer(loadopt, key):
    ...
    """
    return loadopt.set_column_strategy((key,), {"deferred": True, "instrument": True})
