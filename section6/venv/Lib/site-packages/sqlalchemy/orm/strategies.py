...
class LazyLoader(AbstractRelationshipLoader, util.MemoizedSlots):
    ...
    def _load_for_state(self, state, passive):
        ...
        return self._emit_lazyload(session, state, primary_key_identity, passive)
    ...
    def _emit_lazyload(
        ...
        if self.parent_property.secondary is not None:
            q.add_criteria(
                lambda q: q.select_from(self.mapper, self.parent_property.secondary)
            )
        ...
        if self.use_get:
            ...
            return (
                ...
                ._load_on_pk_identity(session.query(self.mapper), primary_key_identity)
            )

        if self.parent_property.order_by:
            q.add_criteria(
                lambda q: q.order_by(*util.to_list(self.parent_property.order_by))
            )
...
class SubqueryLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs
        ...
        with_poly_info = path.get(context.attributes, "path_with_polymorphic", None)
    ...
    def _get_leftmost(self, subq_path):
        ...
        leftmost_attr = [
            getattr(subq_path[0].entity, leftmost_mapper._columntoproperty[c].key)
    ...
    def create_row_processor(
        ...
        if self.uselist:
            self._create_collection_loader(context, collections, local_cols, populators)
        else:
            self._create_scalar_loader(context, collections, local_cols, populators)

    def _create_collection_loader(self, context, collections, local_cols, populators):
    ...
    def _create_scalar_loader(self, context, collections, local_cols, populators):
...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs
        ...
        user_defined_adapter = (
            self._init_user_defined_eager_proc(loadopt, context) if loadopt else False
        )
        ...
        if with_poly_info is not None and None in set(context.secondary_columns):
            raise sa_exc.InvalidRequestError(
    ...
    def _init_user_defined_eager_proc(self, loadopt, context):
        ...
        if alias is not None:
            ...
            if path.contains(context.attributes, "path_with_polymorphic"):
                with_poly_info = path.get(context.attributes, "path_with_polymorphic")
        ...
        path.set(context.attributes, "user_defined_eager_row_processor", adapter)
    ...
    def _create_eager_join(
        ...
        should_nest_selectable = (
            context.multi_row_eager_loaders and context.query._should_nest_selectable
        )
        ...
        if attach_on_outside:
            ...
            eagerjoin = self._splice_nested_inner_join(path, towrap, clauses, onclause)
    ...
    def create_row_processor(
        ...
        if eager_adapter is not False:
            ...
            _instance = loading._instance_processor(
                self.mapper, context, result, our_path[self.mapper], eager_adapter
            )
...
class SelectInLoader(AbstractRelationshipLoader, util.MemoizedSlots):
    ...
    @util.dependencies("sqlalchemy.ext.baked")
    def _load_for_path(self, baked, context, path, states, load_only, effective_entity):

        if load_only and self.key not in load_only:
            return

        our_states = [(state.key[1], state, overwrite) for state, overwrite in states]
        ...
        q.add_criteria(
            lambda q: q.select_from(pa)
            .join(getattr(pa, self.parent_property.key).of_type(effective_entity))
            .filter(self._in_expr.in_(sql.bindparam("primary_keys", expanding=True)))
            .order_by(*pk_cols)
        )

        orig_query = context.query

        q._add_lazyload_options(orig_query._with_options, path[self.parent_property])
