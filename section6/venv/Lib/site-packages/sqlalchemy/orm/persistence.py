...
def _bulk_insert(
    mapper, mappings, session_transaction, isstates, return_defaults, render_nulls
):
...
def _collect_insert_commands(
    table, states_to_insert, bulk=False, return_defaults=False, render_nulls=False
):
...
def _collect_update_commands(uowtransaction, table, states_to_update, bulk=False):
    ...
    for state, state_dict, mapper, connection, update_version_id in states_to_update:
        ...
        if bulk:
            ...
            if mapper.base_mapper.eager_defaults:
                has_all_defaults = mapper._server_onupdate_default_cols[table].issubset(
                    params
                )
            else:
                has_all_defaults = True
    ...
    for (
        ...
        if (
            ...
            for (
                ...
                if value_params:
                    result = connection.execute(statement.values(value_params), params)
                else:
...
def _cached_connection_dict(base_mapper):
    ...
    return util.PopulateDict(
        lambda conn: conn.execution_options(compiled_cache=base_mapper._compiled_cache)
    )
...
def _sort_states(states):
    ...
    return sorted(pending, key=operator.attrgetter("insert_order")) + sorted(
        persistent, key=lambda q: q.key[1]
    )
...
class BulkEvaluate(BulkUD):
    ...
    def _do_pre_synchronize(self):
        ...
        self.matched_objects = [
            obj
            for (cls, pk, identity_token), obj in query.session.identity_map.items()
...
class BulkUpdate(BulkUD):
    ...
    def _do_exec(self):
        ...
        update_stmt = sql.update(
            self.primary_table, self.context.whereclause, values, **self.update_kwargs
        )
...
class BulkUpdateEvaluate(BulkEvaluate, BulkUpdate):
    ...
    def _do_post_synchronize(self):
        ...
        for obj in self.matched_objects:
            ...
            state._expire_attributes(dict_, set(evaluated_keys).difference(to_evaluate))
            states.add(state)
