...
class Mapper(InspectionAttr):
            ...
            if self.local_table is None:
                ...
                if self.concrete:
                    ...
                    self._inherits_equated_pairs = sql_util.criterion_as_pairs(
                        self.mapped_table.onclause, consider_as_foreign_keys=fks
                    )
            else:
                self.mapped_table = self.local_table
    ...
    def _configure_legacy_instrument_class(self):
        ...
        if self.inherits:
            ...
            super_extensions = set(
                chain(
                    *[m._deprecated_extensions for m in self.inherits.iterate_to_root()]
                )
            )
        else:
            super_extensions = set()
        ...
        if manager is not None:
            ...
            if manager.is_mapped:
                raise sa_exc.ArgumentError(
                    ...
                    "remove *all* current mappers from all classes." % self.class_
                )
            # else:
        ...
        if setter:
            ...
            def _set_polymorphic_identity(state):
                ...
                state.get_impl(polymorphic_key).set(
                    state, dict_, state.manager.mapper.polymorphic_identity, None
                )
    ...
    def _configure_property(self, key, prop, init=True, setparent=True):
        ...
        if (
            ...
            and not isinstance(
                self._props[key],
                (properties.ColumnProperty, properties.ConcreteInheritedProperty),
            )
        ):
            util.warn(
    ...
    def _property_from_column(self, key, prop):
        ...
        if not expression._is_column(column):
            raise sa_exc.ArgumentError(
                "%s=%r is not an instance of MapperProperty or Column" % (key, prop)
            )
    ...
    def _propkey_to_col(self):
        return dict(
            (table, dict((self._columntoproperty[col].key, col) for col in columns))
            for table, columns in self._cols_by_table.items()
        )
        ...
        def visit_binary(binary):
            ...
            if leftcol.table not in tables:
                leftval = self._get_committed_state_attr_by_column(
                    state, state.dict, leftcol, passive=attributes.PASSIVE_NO_INITIALIZE
                )
    ...
    @util.dependencies("sqlalchemy.ext.baked", "sqlalchemy.orm.strategy_options")
    def _subclass_load_via_in(self, baked, strategy_options, entity):
        ...
        if entity.is_aliased_class:
            ...
            q = baked.BakedQuery(
                self._compiled_cache, lambda session: session.query(self), (self,)
            )
        ...
        for table in self._sorted_tables:
            ...
            for m in self.iterate_to_root():
                if m._inherits_equated_pairs and cols.intersection(
                    util.reduce(
                        set.union, [l.proxy_set for l, r in m._inherits_equated_pairs]
                    )
                ):
