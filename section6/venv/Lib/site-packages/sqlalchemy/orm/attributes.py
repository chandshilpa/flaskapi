...
class QueryableAttribute(
    ...
    def __init__(
        self, class_, key, impl=None, comparator=None, parententity=None, of_type=None
    ):
        self.class_ = class_
        self.key = key
        self.impl = impl
...
class Event(object):
    ...
    def __eq__(self, other):
        return (
            isinstance(other, Event) and other.impl is self.impl and other.op == self.op
        )
    ...
    def set(
        ...
        if self.dispatch._active_history:
            ...
            old = self.get(
                state,
                dict_,
                passive=PASSIVE_NO_FETCH ^ INIT_OK | LOAD_AGAINST_COMMITTED,
            )
...
def backref_listeners(attribute, key, uselist):
    ...
    if uselist:
        ...
        event.listen(
            attribute, "set", emit_backref_from_scalar_set_event, retval=True, raw=True
        )
...
def get_history(obj, key, passive=PASSIVE_OFF):
    ...
    if passive is True:
        ...
        util.warn_deprecated(
            "Passing False for 'passive' is " "deprecated.  Use attributes.PASSIVE_OFF"
        )
        passive = PASSIVE_OFF
