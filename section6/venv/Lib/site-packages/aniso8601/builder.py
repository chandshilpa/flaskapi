    ...
    def _build_object(cls, parsetuple):
        ...
        if parsetuple[-1] == "date":
            ...
            return cls.build_time(
                hh=parsetuple[0], mm=parsetuple[1], ss=parsetuple[2], tz=parsetuple[3]
            )
            ...
            return cls.build_interval(
                start=parsetuple[0], end=parsetuple[1], duration=parsetuple[2]
            )
        elif parsetuple[-1] == "repeatinginterval":
            return cls.build_repeating_interval(
                R=parsetuple[0], Rnn=parsetuple[1], interval=parsetuple[2]
            )

        return cls.build_timezone(
            negative=parsetuple[0],
            Z=parsetuple[1],
            hh=parsetuple[2],
            mm=parsetuple[3],
            name=parsetuple[4],
...
class PythonTimeBuilder(BaseTimeBuilder):
    ...
    def build_date(cls, YYYY=None, MM=None, DD=None, Www=None, D=None, DDD=None):
        ...
        if Www is not None:
            weeknumber = BaseTimeBuilder.cast(
                Www, int, thrownmessage="Invalid week string."
            )
        ...
        if DDD is not None:
            dayofyear = BaseTimeBuilder.cast(
                DDD, int, thrownmessage="Invalid day string."
            )
        else:
            dayofyear = None

        if D is not None:
            dayofweek = BaseTimeBuilder.cast(
                D, int, thrownmessage="Invalid day string."
            )
    ...
    def build_time(cls, hh=None, mm=None, ss=None, tz=None):
        ...
        if (
            hours == 24
            and floathours == 0
            and (minutes != 0 or floatminutes != 0 or seconds != 0 or floatseconds != 0)
        ):
        ...
        if tz is not None:
            return (
                ...
                + datetime.timedelta(
                    hours=floathours, minutes=floatminutes, seconds=floatseconds
                )
            ).timetz()

        return (
            datetime.datetime(1, 1, 1, hour=hours, minute=minutes, second=seconds)
            + datetime.timedelta(
                hours=floathours, minutes=floatminutes, seconds=floatseconds
            )
        ).time()
    ...
    def build_duration(
        ...
        if PnY is not None:
            years = BaseTimeBuilder.cast(
                PnY, float, thrownmessage="Invalid year string."
            )

        if PnM is not None:
            months = BaseTimeBuilder.cast(
                PnM, float, thrownmessage="Invalid month string."
            )
        if PnD is not None:
        ...
        if PnW is not None:
            if "." in PnW:
                weeks = BaseTimeBuilder.cast(
                    PnW, float, thrownmessage="Invalid week string."
                )
            else:
                weeks = BaseTimeBuilder.cast(
                    PnW, int, thrownmessage="Invalid week string."
                )

        if TnH is not None:
            if "." in TnH:
                hours = BaseTimeBuilder.cast(
                    TnH, float, thrownmessage="Invalid hour string."
                )
            else:
                hours = BaseTimeBuilder.cast(
                    TnH, int, thrownmessage="Invalid hour string."
                )

        if TnM is not None:
            if "." in TnM:
                minutes = BaseTimeBuilder.cast(
                    TnM, float, thrownmessage="Invalid minute string."
                )
            else:
                minutes = BaseTimeBuilder.cast(
                    TnM, int, thrownmessage="Invalid minute string."
                )
        ...
        if TnS is not None:
            if "." in TnS:
                ...
                seconds = BaseTimeBuilder.cast(
                    TnS, int, thrownmessage="Invalid second string."
                )
        ...
        return datetime.timedelta(
            days=totaldays, seconds=seconds, minutes=minutes, hours=hours, weeks=weeks
        )
...
class RelativeTimeBuilder(PythonTimeBuilder):
    ...
    def build_duration(
        ...
        try:
            ...
            raise RuntimeError(
                "dateutil must be installed for " "relativedelta support."
            )
        ...
        if (PnY is not None and "." in PnY) or (PnM is not None and "." in PnM):
            ...
            raise RelativeValueError(
                "Fractional months and years are not " "defined for relative durations."
            )
        ...
        if PnM is not None:
            months = BaseTimeBuilder.cast(
                PnM, int, thrownmessage="Invalid month string."
            )
        ...
        if TnS is not None:
            if "." in TnS:
                ...
                seconds = BaseTimeBuilder.cast(
                    TnS[0 : TnS.index(".")], int, thrownmessage="Invalid second string."
                )

                # Truncate to maximum supported precision
                microseconds = (
                    BaseTimeBuilder.cast(
                        TnS[TnS.index(".") : TnS.index(".") + 7],
                        float,
                        thrownmessage="Invalid second string.",
                    )
                    * 1e6
