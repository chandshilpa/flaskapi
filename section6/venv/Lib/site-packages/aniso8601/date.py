...
def get_date_resolution(isodatestr):
    ...
    raise ISOFormatError(
        '"{0}" is not an ISO 8601 date, perhaps it '
...
def _parse_calendar_day(datestr, builder):
    ...
    if len(datestr) == 10:
        ...
        raise ISOFormatError(
            '"{0}" is not a valid ISO 8601 calendar day.'.format(datestr)
        )
...
def _parse_calendar_month(datestr, builder):
    ...
    if len(datestr) != 7:
        raise ISOFormatError(
            '"{0}" is not a valid ISO 8601 calendar month.'.format(datestr)
        )
